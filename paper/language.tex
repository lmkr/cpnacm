...
produce tokens on the places which will change the \concept{current
...
the associated \figitem{CollectVotes} submodule. This illustrates how
use of ordinary and substitution transitions can be mixed within a
module. We omit the details of \figitem{CollectVotes} in this paper.
...
\figitem{Idle} and \figitem{WaitingDecision} model the two main
...
Fig.~\ref{fig:worker} is \smlcode{1`wrk(1) ++ 1`wrk(2)} modelling a
...
reflecting that each of the two workers may vote \smlcode{Yes} or
\smlcode{No} to committing the transactions:
...
place \figitem{Votes} to inform the coordinator that worker 1 votes
