...
(left) containing the \figitem{Commit} module and one binder (right)
contained the \figitem{Coordinator} module. In addition, two tool
palettes are shown: one (\figitem{Sim}) containing the tools that can
be used for simulation of the model, and one tool palette
(\figitem{Create}) containing the tools for creating CPN model
...
\begin{figure*}[t]
\centering
\includegraphics[scale=.35]{figures/cpntools.png}
