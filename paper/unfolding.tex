...
subnet, the CPN place \figitem{CanCommit} needs to be unfolded to two
PTN places corresponding to \smlcode{wrk(1)} and \smlcode{wrk(2)}. The
place \figitem{Idle} and \figitem{WaitingVotes} does not need to be
unfolded as the \smlcode{Unit} color set contains just a single
...
just need to change the symbolic constant \smlcode{W}
...
modeling (as we only need a single instance of the \figitem{CanCommit}
